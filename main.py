...
#Setting an HTTPS Context to fetch data 
...
def liked_movie():
    ...
    return jsonify({
        "status": "success",
        "total liked": len(liked_movies),
        "total_movies_now":len(all_movies),
    }), 201
...
def unliked_movie():
    ...
    return jsonify({
        "status": "success",
        "total unliked": len(not_liked_movies),
        "total_movies_now":len(all_movies),
    }), 201
...
def did_not_watch_view():
    ...
    return jsonify({
        "status": "success",
        "total movies not watched": len(did_not_watch),
        "total_movies_now":len(all_movies),
    }), 201
